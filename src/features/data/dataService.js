...
const TRANSACTION_DAILY_API = `${BILLER_DATA_API}/biller_group_admin/total_transactions?group_by=day`
...
const getDailyTransaction = async (token) => {
  const config = {
    headers: {
      Authorization: `Bearer ${token}`
    }
  ...
  // send get request to server
  const response = await axios.get(TRANSACTION_DAILY_API, config)
  return response.data
}
